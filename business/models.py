...
class Magasin(models.Model):
    ...
    adress      = models.CharField(verbose_name="Adresse", max_length=150, blank=True)
    principale   = models.BooleanField(verbose_name='Magasin principale', default=False)
    def save(self, *args, **kwargs):
        if self.principale == True:
            self.principale = True
            Magasin.objects.all().exclude(id=self.id).update(principale=False)
        return super(Magasin, self).save(*args, **kwargs)
    def __str__(self):
        return self.name